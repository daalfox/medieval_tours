...
func (r PgRepo) List(ctx context.Context) ([]ScheduleWithId, error) {
	...
	schedules, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (ScheduleWithId, error) {
		s := ScheduleWithId{}
		if err := row.Scan(&s.Id, &s.Schedule.TourId, &s.Schedule.Price, &s.Schedule.StartsAt); err != nil {
			return s, err
		}
		return s, nil
	})
...
func (r PgRepo) Insert(ctx context.Context, schedule Schedule) int64 {
	var id int64
	err := r.pool.QueryRow(ctx, "insert into schedule(tour_id, price, starts_at) values($1, $2, $3) returning id", schedule.TourId, schedule.Price, schedule.StartsAt).Scan(&id)
	if err != nil {
		log.Println(err)
	}
